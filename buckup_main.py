...
def main(argv=None):
    options = PipelineOptions(
        ...
        service_account_key_file=temp_file_path,
        num_workers=1,
        max_num_workers=3,
    ...
    with beam.Pipeline(options=options) as p1:
        get_names = (
            p1
            | 'Create get names' >> beam.Create([None])
            | 'Execute Get Names' >> beam.ParDo(GetNames())
        )

        get_tables = (
            get_names
            | 'Execute Get Tables' >> beam.ParDo(GetTables())
        )
        
        tables_tranform_bq = (
            get_names
            | 'Execute Tables Transform BQ' >> beam.ParDo(TablesTransformBQ())
        )
